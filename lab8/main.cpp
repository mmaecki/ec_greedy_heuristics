...
int main1(){
    ...
    vector<SearchType> searchTypes = {greedy};
            ...
            for(auto initialSolutionType: initialSolutionTypes){
                for(auto interNeighbourhoodType: interNeighbourhoodTypes){
                    ...
                    for(int i=0; i<1000; i++){
                        int startNode = rand() % distances.size();
                        LocalSearch ls = LocalSearch(searchType, initialSolutionType, interNeighbourhoodType, distances, costs, startNode);
                        Result result = ls.solve();
                        algoResults.push_back(result);
                        vector<int> solution = result.bestSolution;
                    ...
                    //algoResults to json
                    string json = "{\n";
                    json += "\t\"name\": \"" + LocalSearch(searchType, initialSolutionType, interNeighbourhoodType, distances, costs, 0).name + "\",\n";
                    json += "\t\"problem\": \"" + ProblemInstanceStrings[problemInstance] + "\",\n";
                    json += "\t\"results\": [\n";
                    for(int i=0; i<algoResults.size(); i++){
                        json += "\t\t{\n";
                        json += "\t\t\t\"cost\": " + to_string(algoResults[i].bestCost) + ",\n";
                        json += "\t\t\t\"bestSolution\": [\n";
                        for(int j=0; j<algoResults[i].bestSolution.size(); j++){
                            json += "\t\t\t\t" + to_string(algoResults[i].bestSolution[j]) + ",\n";
                        }
                ...
                json += "\t]\n";
                json += "}\n";
                string filename = "results/" + ProblemInstanceStrings[problemInstance] + ".json";
                ofstream file;
                file.open(filename);
                file << json;
                file.close();
